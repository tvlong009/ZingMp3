...
$(document).ready(function(){$("#link-get-active-code").each(function(){var self=this;var url=$(self).attr('href');$(this).overlay({mask:{top:200,color:'#ccc',loadSpeed:50,opacity:0.5},onBeforeLoad:function(){var wrap=this.getOverlay().find(".contentWrap");wrap.html('');$.get(url,{msisdn:$('#vt_signup_msisdn').val()},function(data){wrap.html(data);});},onClose:function(){$('#signUpCaptcha a img#captcha_image').click();}});return false;});$('#get-active-code-submit').live('click',function(){var self=$(this);var requestUrl=self.attr('vthref');var msisdn=$('#vt_get_active_code_msisdn').val()
if(self.hasClass('_disable'))
{return false;}
self.addClass('_disable');$('#vt_get_active_code_msisdn').attr('disabled','disabled');var vt_get_active_code={msisdn:msisdn,captcha:$('#vt_get_active_code_captcha').val(),_csrf_token:$('#vt_get_active_code__csrf_token').val()};$.post(requestUrl,{vt_get_active_code:vt_get_active_code,confirm:1},function(data)
{if($.trim(data)!='ok')
$('#overlay-login .contentWrap').html(data);else
{$('#vt_signup_msisdn').val(msisdn);$('#pop-up-content').html('<p class="notice">Vui lòng kiểm tra tin nhắn để lấy mã xác nhận</p>');}});return false;});});
$(function(){$("a.link-reg-confirm").each(function(){var self=this;$(this).overlay({mask:{top:200,color:'#ccc',loadSpeed:50,opacity:0.5},onBeforeLoad:function(){var wrap=this.getOverlay().find(".contentWrap");wrap.html('');href=$(self).attr('href');wrap.load(href);}});return false;});$('a.reg_confrim_close').live('click',function(){$("a.link-reg-confirm").each(function(){$(this).overlay().close();});return false;});$('.btn-reg-confirm').live('click',function(){var requestUrl=$('#vt_form_reg-confirm').attr('action');$('#vt_reg_confirm_msisdn').attr('disabled','disabled');$('#vt_reg_confirm_active_code').attr('disabled','disabled');$("input.btn-reg-confirm").attr("disabled","disabled");var reg_confirm_data={msisdn:$('#vt_reg_confirm_msisdn').val(),active_code:$('#vt_reg_confirm_active_code').val(),_csrf_token:$('#vt_reg_confirm__csrf_token').val()};$.post(requestUrl,{vt_reg_confirm:reg_confirm_data},function(data)
{if($.trim(data)!='ok'){$('#overlay-login .contentWrap').html(data);$("input.btn-reg-confirm").removeAttr("disabled");}
else
location.reload();});return false;});$('.btn-add-info').live('click',function(){var requestUrl=$('#vt_form_add_info').attr('action');$('#vt_add_info_form_username').attr('disabled','disabled');$('#vt_add_info_form_password').attr('disabled','disabled');$('#vt_add_info_form_password_again').attr('disabled','disabled');$('#vt_add_info_form_email').attr('disabled','disabled');$("input.btn-add-info").attr("disabled","disabled");var add_infomation={username:$('#vt_add_info_form_username').val(),password:$('#vt_add_info_form_password').val(),password_again:$('#vt_add_info_form_password_again').val(),email:$('#vt_add_info_form_email').val(),captcha:$('#vt_add_info_form_captcha').val(),_csrf_token:$('#vt_add_info_form__csrf_token').val()};$.post(requestUrl,{vt_add_info_form:add_infomation},function(data)
{if($.trim(data)!='ok'){$('#overlay-login .contentWrap').html(data);$("input.btn-add-info").removeAttr("disabled");}
else{alert('Chào mừng bạn đã tham gia vào hệ thống Keeng');location.href='/';}});return false;});});;
