...
$(function(){$("a.btn_download_ringtone").each(function(){var self=this;if($(self).hasClass('_disable_button'))
{return false;}
$(this).overlay({mask:{top:200,color:'#ccc',loadSpeed:50,opacity:0.5},onBeforeLoad:function(){var ringtoneId;ringtoneId=$(self).attr('id');var wrap=this.getOverlay().find(".contentWrap");wrap.html('');var href=$(self).attr('href');if(ringtoneId)
{href+='?ringtone_id='+ringtoneId;}
wrap.load(href);}});});$("a.btn_set_rbt").each(function(){var self=this;if($(self).hasClass('_disable_button'))
{return false;}
...
{href+='?ringbacktone_id='+ringbacktoneId;}
wrap.load(href);}});});$("a.link_share").each(function(){var self=this;$(this).overlay({mask:{top:200,color:'#ccc',loadSpeed:50,opacity:0.5},onBeforeLoad:function(){var wrap=this.getOverlay().find(".contentWrap");wrap.html('');var href=$(self).attr('href');wrap.load(href);}});});$("a.btn_gift_rbt").each(function(){var self=this;if($(self).hasClass('_disable_button'))
{return false;}
$(this).overlay({mask:{top:200,color:'#ccc',loadSpeed:50,opacity:0.5},onBeforeLoad:function(){var ringbacktoneId;ringbacktoneId=$(self).attr('rbt');var wrap=this.getOverlay().find(".contentWrap");wrap.html('');var href=$(self).attr('href');if(ringbacktoneId)
{href+='?ringbacktone_id='+ringbacktoneId;}
wrap.load(href);}});});$("a.btn_gift_ringtone").each(function(){var self=this;if($(self).hasClass('_disable_button'))
{return false;}
$(this).overlay({mask:{top:200,color:'#ccc',loadSpeed:50,opacity:0.5},onBeforeLoad:function(){var ringtoneId;ringtoneId=$(self).attr('id');var wrap=this.getOverlay().find(".contentWrap");wrap.html('');var href=$(self).attr('href');if(ringtoneId)
{href+='?ringtone_id='+ringtoneId;}
wrap.load(href);}});});$('#embed').click(function(){if($('.click-embed').is(":visible")){$('.click-embed').addClass('_hidden');$('.click-add').addClass('_hidden');}
else
{$('.click-embed').removeClass('_hidden');$('.click-add').addClass('_hidden');}});});;
...
(function($){var keengVideoPlayer=jwplayer(videoPlayerName).setup({flashplayer:"/swf/playervideo.swf",file:videoPlayerLink,image:$.base64Decode(videoImageLink),autostart:true,streamer:_streamer,startparam:_startparam,type:_type,loop:false,height:396,width:641,wmode:"transparent",screencolor:'000000',allowfullscreen:true,usefullscreen:true,skin:'/swf/video-magenta.swf',"controlbar.position":"bottom","controlbar.idlehide":"false",events:{onComplete:function(event){$('#relate').fadeIn(1000);},onPlay:function(event){document.getElementById('relate').style.display='none';}}});insertPlayerIntoPage(videoPlayerName);keengVideoPlayer.onPlay(function(event){pauseAllPlayerInPageWithout(videoPlayerName);});$('.link-replay').live("click",function(){keengVideoPlayer.play();})
$('#embed').click(function()
{$('input.embed-video').attr('value'," <embed type='application/x-shockwave-flash' allowscriptaccess='none'  width='100%' height='100%' src='http://www.keeng.vn/swf/playervideo.swf?file="+videoPlayerLink+"&amp;image="+$.base64Decode(videoImageLink)+"&amp;loop=false&amp;wmode=transparent&amp;screencolor=000000&amp;allowfullscreen=true&amp;usefullscreen=true&amp;skin=http://www.keeng.vn/swf/video-magenta.swf&amp;controlbar.position=bottom&amp;controlbar.idlehide=false'>")});})(jQuery);;
