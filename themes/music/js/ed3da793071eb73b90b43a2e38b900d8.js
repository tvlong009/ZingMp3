...
var hiddenPlayer=null;var isFirstError=false;(function($){$('div.hiddenPlayerMagic').each(function(){playerDivId=$(this).attr('id');hiddenPlayer=jwplayer(playerDivId).setup({flashplayer:"/swf/player.swf",file:"/swf/So Close.mp3",height:1,loop:true,repeat:'always',width:1,screencolor:'000000'});hidePlayer(playerDivId);insertPlayerIntoPage(playerDivId);jwplayer(playerDivId).stop();});})(jQuery);function hidePlayer(hiddenPlayerName){if($('div#'+hiddenPlayerName+'_wrapper').attr("id")==hiddenPlayerName
+'_wrapper'){$('div#'+hiddenPlayerName+'_wrapper').css('position','absolute');$('div#'+hiddenPlayerName+'_wrapper').css('top','1px');$('div#'+hiddenPlayerName+'_wrapper').css('left','1px');}else{setTimeout('hidePlayer("'+hiddenPlayerName+'")',100);}}
function hiddenPlayerHandlers(hiddenPlayerName){jwplayer(hiddenPlayerName).onError(function(event){if(isFirstError){$('.panel'+hiddenPlayerName+' li.playing').removeClass('playing');jwplayer(hiddenPlayerName).stop();isFirstError=false;alert('Bài hát bị lỗi!');}});jwplayer(hiddenPlayerName).onTime(function(event){dur=event.duration;pos=event.position;$(currentProcessBar).width(""+Math.floor(currentMaxWidth*pos/dur)+"px");});jwplayer(hiddenPlayerName).onPause(function(event){$('.panel'+hiddenPlayerName+' li.parent-player a.pause').removeClass('pause');});jwplayer(hiddenPlayerName).onPlay(function(event){$('.panel'+hiddenPlayerName+' .control-media3 p.left').width("0px");$('.panel'+hiddenPlayerName+' li.parent-player').removeClass('playing');$(currentParentPlay).addClass('playing');$(currentBtnPlay).addClass('pause');pauseAllPlayerInPageWithout(hiddenPlayerName);});jwplayer(hiddenPlayerName).onComplete(function(){$('.panel'+hiddenPlayerName+' .control-media3 p.left').width("0px");$('.panel'+hiddenPlayerName+' li.playing').removeClass('playing');$('.panel'+hiddenPlayerName+' li.parent-player a.pause').removeClass('pause');});}
var currentMaxWidth=0;var currentProcessBar='';var currentBtnPlay='';var currentPlayerId='';var currentParentPlay='';function playMusicHidden(hiddenPlayerName,id,link,max_width){$('div#'+hiddenPlayerName+'_wrapper').css('top','0px');$('div#'+hiddenPlayerName+'_wrapper').css('top','1px');isFirstError=true;currentMaxWidth=max_width;currentProcessBar='.iproccess'+id;currentBtnPlay='a.iplay'+id;currentParentPlay='li.iparent'+id;if($(currentBtnPlay).hasClass('pause')&&$(currentParentPlay).hasClass('playing')){jwplayer(hiddenPlayerName).pause(true);}else{state=jwplayer(hiddenPlayerName).getState();if(state=="PAUSED"&&$(currentParentPlay).hasClass('playing')){jwplayer(hiddenPlayerName).play(true);}else if(state=="BUFFERING"){}else{jwplayer(hiddenPlayerName).stop();$('.panel'+hiddenPlayerName+' li.playing').removeClass('playing');$(currentParentPlay).addClass('playing');$(currentBtnPlay).addClass('pause');jwplayer(hiddenPlayerName).load($.base64Decode(link));hiddenPlayerHandlers(hiddenPlayerName);$('.panel'+hiddenPlayerName+' .control-media3 p.left').width("0px");jwplayer(hiddenPlayerName).play(true);}}
currentPlayerId=id;};
