...
$(function(){$("a.btn_set_rbt").each(function(){var self=this;var url=$(self).attr('vthref');$(this).overlay({mask:{top:200,color:'#ccc',loadSpeed:50,opacity:0.5},onBeforeLoad:function(){var songId;songId=$('ul.list-song li.playing').attr('playingId');var wrap=this.getOverlay().find(".contentWrap");wrap.html('');$.post(url,{song_id:songId},function(data){wrap.html(data);});}});});$('.box-popup-content h1 a').live('click',function(){$("a.btn_set_rbt").each(function(){$(this).overlay().close();});return false;});$("a.btn_gift_rbt").each(function(){var self=this;var url=$(self).attr('vthref');$(this).overlay({mask:{top:200,color:'#ccc',loadSpeed:50,opacity:0.5},onBeforeLoad:function(){var songId;songId=$('ul.list-song li.playing').attr('playingId');var wrap=this.getOverlay().find(".contentWrap");wrap.html('');$.post(url,{song_id:songId},function(data){wrap.html(data);});}});});$('#present-ringbacktone-submit').live('click',function(){$(this).addClass('_disable');var url=$(this).attr('href');var self=$(this);$.post(url,{confirm:1,receiver_msisdn:$('#receiver_msisdn').val()},function(response){$('#overlay-login .contentWrap').html(response);return false;});return false;});$('.box-popup-content h1 a').live('click',function(){$("a.btn_gift_rbt").each(function(){$(this).overlay().close();});return false;});$("a.btn_set_rt").each(function(){var self=this;var url=$(self).attr('vthref');$(this).overlay({mask:{top:200,color:'#ccc',loadSpeed:50,opacity:0.5},onBeforeLoad:function(){var songId;songId=$('ul.list-song li.playing').attr('playingId');var wrap=this.getOverlay().find(".contentWrap");wrap.html('');$.post(url,{song_id:songId},function(data){wrap.html(data);});}});});$("a.btn_gift_rt").each(function(){var self=this;var url=$(self).attr('vthref');$(this).overlay({mask:{top:200,color:'#ccc',loadSpeed:50,opacity:0.5},onBeforeLoad:function(){var songId;songId=$('ul.list-song li.playing').attr('playingId');var wrap=this.getOverlay().find(".contentWrap");wrap.html('');$.post(url,{song_id:songId},function(data){wrap.html(data);});}});});$('#present-ringtone-submit').live('click',function(){$(this).addClass('_disable');var url=$(this).attr('href');var self=$(this);$.post(url,{confirm:1,receiver_msisdn:$('#receiver_msisdn').val()},function(response){$('#overlay-login .contentWrap').html(response);return false;});return false;});$("a.link_share").each(function(){var self=this;$(this).overlay({mask:{top:200,color:'#ccc',loadSpeed:50,opacity:0.5},onBeforeLoad:function(){var wrap=this.getOverlay().find(".contentWrap");wrap.html('');var href=$(self).attr('href');wrap.load(href);}});});});;
var songIndex=0;var albumPlayer=null;var listenno=0;(function($){albumPlayer=jwplayer(playerName).setup({flashplayer:"/swf/mp3player.swf",file:$.base64Decode(albumPlayerXml),height:37,width:641,repeat:'always',wmode:"transparent",screencolor:'ffffff',bgcolor:'ffffff',skin:'/swf/mp3-playlist-magenta.swf',"controlbar.position":"top","controlbar.idlehide":"false",autoplay:player_autoplay,events:{onComplete:function(){if(rewind)albumPlayer.seek(0);}}});insertPlayerIntoPage(playerName);albumPlayer.onComplete(function(){$(".list-song li.playing").removeClass("playing");$(".viewLyric").removeClass("viewLyric").addClass("hiddenLyric");});albumPlayer.onError(function(event){$("ul.list-song li.playing").removeClass("playing");$(".viewLyric").removeClass("viewLyric").addClass("hiddenLyric");albumPlayer.playlistNext();});albumPlayer.onPlay(function(event){pauseAllPlayerInPageWithout(playerName);if(listenno==0){var number=parseInt($('span.number_album').text());number++;$('span.number_album').text(number);}
listenno++;});albumPlayer.onPlaylistItem(function(event){index=event.index;var idCurr=albumPlayer.getPlaylistItem(event.index).link;$(".viewLyric").removeClass("viewLyric").addClass("hiddenLyric");$(".list-song li.playing").removeClass("playing");$("ul.list-song li:eq("+index+")").addClass("playing");$("#lyricId"+index).removeClass('hiddenLyric').addClass("viewLyric");});$('#scrollbar1').tinyscrollbar();$('#embed').click(function()
{$('input.embed-album').attr('value',"<embed type='application/x-shockwave-flash' allowscriptaccess='none'  width='100%' height='37' src='http://www.keeng.vn/swf/mp3player.swf?file=http://keeng.vn"+$.base64Decode(albumPlayerXml)+"&amp;wmode=transparent&amp;screencolor=000000&amp;allowfullscreen=true&amp;usefullscreen=true&amp;skin=http://www.keeng.vn/swf/mp3-playlist-magenta.swf&amp;repeat=always&amp;controlbar.position=top&amp;controlbar.idlehide=false'>")});})(jQuery);function onRewind(r){rewind=r;}
function playSong(songid){var playList=albumPlayer.getPlaylist();for(var index in playList){if(songid==playList[index].link){albumPlayer.playlistItem(index);albumPlayer.play(true);break;}}};;
