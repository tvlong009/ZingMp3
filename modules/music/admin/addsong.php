...
$array['cid'] = $nv_Request->get_int( 'idcountry', 'post', 0 );
...
$sql = "SELECT * FROM `" . NV_PREFIXLANG . "_" . $module_data . "_country`";
$result = $db->sql_query($sql);
while( $row = $db->sql_fetchrow( $result,2 ) )
{
	$si = ( $array['cid'] == $row['id'] ) ?  "selected=\"selected\"" : '' ;
	$contents .= "<option " . $si . " value=\"" . $row['id'] . "\" >" . $row['title'] . "</option>\n";
}
$contents .= "</select>
