...
$countryid = $nv_Request->get_int( 'cid', 'get', 0 );
$country = "SELECT title FROM `" . NV_PREFIXLANG . "_" . $module_data . "_country` WHERE `id`=" . $countryid;
$countryname = $db->sql_fetchrow($db->sql_query($country));
$page_title = $lang_module['content_list']." ".$countryname['title'];
...
$where ="WHERE a.cid=" .$countryid;
$where .= empty( $theloai ) ? ' ' : ' AND ';
if( ! empty( $q ) )
{
	if( $type_search == 'ten' )
	{
...
$link = "index.php?" . NV_NAME_VARIABLE . "=" . $module_name ."&".NV_OP_VARIABLE."=country_song&cid=".$countryid. "&numshow=" . $numshow . "&where_search=" . $where_search . "&type_search=" . $type_search . "&q=" . $q . "&order=" . $order;
//die($sql);
$num = $db->sql_query( $sqlnum );
$output = $db->sql_numrows( $num );
$ts = ceil( $output / $numshow );
//die($countryid."das");
$xtpl = new XTemplate( "country_song.tpl", NV_ROOTDIR . "/themes/" . $global_config['module_theme'] . "/modules/" . $module_name );
$xtpl->assign( 'cid',$countryid);
...
$xtpl->assign( 'FORM_ACTION', NV_BASE_ADMINURL . 'index.php?'. NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . $module_name ."&cid=".$countryid );
