...
// Xoa chu de bai hat
function nv_delete_country( id ){
	if ( confirm( nv_is_del_confirm[0] ) ){
		nv_ajax( 'get', script_name, nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=country&ac=del&id=' + id, '', 'nv_delete_country_result' );
	}
	return false;
}
function nv_delete_country_result(res){
	alert( res );
	location.reload(); 
	return false;
}
