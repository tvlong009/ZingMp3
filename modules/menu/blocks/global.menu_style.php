...
if (!nv_function_exists('nv_menu_site')) {
    function nv_block_config_menu($module, $data_block, $lang_block)
    {
		...
		$sel = ($data_block['type'] == 9) ? ' selected' : '';
        $html .= "<option value=\"9\" " . $sel . ">" . $lang_block['m_type9'] . "</option>\n";
		$sel = ($data_block['type'] == 10) ? ' selected' : '';
        $html .= "<option value=\"10\" " . $sel . ">" . $lang_block['m_type10'] . "</option>\n";
        $html .= "\t</select></td>\n";
        $html .= "</tr>";
        $html .= "<tr>";
        $html .= "<td>";
    ...
    function nv_menu_site($block_config)
    {
		...
		} elseif ($block_config['type'] == 9) {
            return nv_style_type9($list_cats, $block_config);
		} elseif ($block_config['type'] == 10) {
            return nv_style_type10($list_cats, $block_config);
        } else {
            $style = 'side_menu_bar';
            return nv_style_type($style, $list_cats, $block_config);
        }
	...
	function nv_style_type10($list_cats, $block_config)
    {
        global $module_info;
        if (file_exists(NV_ROOTDIR . "/themes/" . $module_info['template'] . "/modules/menu/menusite.tpl")) {
            $block_theme = $module_info['template'];
        } else {
            $block_theme = "default";
        }
        $xtpl = new XTemplate("menusite.tpl", NV_ROOTDIR . "/themes/" . $block_theme . "/modules/menu");
        $xtpl->assign('BLOCK_THEME', $block_theme);
        $xtpl->assign('NV_BASE_SITEURL', NV_BASE_SITEURL);
        $arr = array();
        foreach ($list_cats as $cat) {
    ...
    function nv_style_type8($list_cats, $block_config)
    {
	 ...
	 function nv_style_type9($list_cats, $block_config)
    {
        global $module_info;
        if (file_exists(NV_ROOTDIR . "/themes/" . $module_info['template'] . "/modules/menu/musicfooter.tpl")) {
            $block_theme = $module_info['template'];
        } else {
            $block_theme = "default";
        }
        $xtpl = new XTemplate("musicfooter.tpl", NV_ROOTDIR . "/themes/" . $block_theme . "/modules/menu");
        $xtpl->assign('BLOCK_THEME', $block_theme);
        $xtpl->assign('NV_BASE_SITEURL', NV_BASE_SITEURL);
        $arr = array();
        foreach ($list_cats as $cat) {
